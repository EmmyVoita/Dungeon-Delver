...
public class GoldenHarvestAbility : AbilityBase
{
    ...
    {
        if (isActive) return;
        AudioHelpers.PlayMyClipAtPoint(activateSound, AudioChannel.SFX, Player.Instance.transform.position);
    ...
    private IEnumerator GoldenHarvestRoutine()
    {
        ...
        effect.GetComponent<WheelAnimatorGroup>().OpenAll();

        isActive = true;
        caughtCount = 0;

        // Start visual effect
        Player.Instance.goal.GetComponent<Goal>().EnterHarvestMode();
        ...
        Player.Instance.goal.GetComponent<Goal>().ExitHarvestMode();
        effect.GetComponent<WheelAnimatorGroup>().CloseAll(true);
